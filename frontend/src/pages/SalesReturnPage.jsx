...
const SalesReturnPage = () => {
  ...
  const fetchReturns = async (reset = false) => {
    ...
    try {
      ...
      const [returnsRes, countRes] = await Promise.all([
        axios.get(`${API}/sales-returns?${params.toString()}`),
        axios.get(`${API}/sales-returns/count?organization_id=${currentOrg.id}${filterStatus !== 'all' ? `&status=${filterStatus}` : ''}${searchTerm ? `&search=${searchTerm}` : ''}`)
      ]);
      if (reset) {
  ...
  const handleItemSelect = (index, item) => {
    ...
    if (formData.credit_account_id && item.id) {
      fetchLastPrice(formData.credit_account_id, item.id, index);
    }
  };
  ...
  const handleSave = async () => {
    if (!formData.debit_account_id || !formData.credit_account_id) {
      toast.error('Please select both Sales Return Account and Customer Account');
      return;
    }
  ...
  const handlePost = async (id) => {
    if (!window.confirm('Post this sales return? This will create a voucher and update inventory.')) return;
    setPosting(true);
    try {
      const res = await axios.post(`${API}/sales-returns/${id}/post`);
  ...
  const handleUnpost = async (id) => {
    ...
    setPosting(true);
    try {
      await axios.post(`${API}/sales-returns/${id}/unpost`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`${API}/sales-returns/${id}`);
  ...
  const fetchCustomerForPrint = async (accountId, accountCode) => {
    try {
      ...
      return { name: '', code: accountCode || '', address: '', registration_number: '', balance_usd: 0, vat_balance_usd: 0 };
    }
  };

  const handlePrint = async (ret) => {
    const customer = await fetchCustomerForPrint(ret.credit_account_id, ret.credit_account_code);
  ...
  return (
    <div className="space-y-6" data-testid="sales-return-page">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage sales returns with inventory restoration</p>
        </div>
        {canEdit() && (
          <Button onClick={openCreateForm} data-testid="create-sales-return-btn">
            <Plus className="w-4 h-4 mr-2" /> New Sales Return
          </Button>
        )}
      </div>
                  ...
                  {returns.map((ret) => (
                    <tr key={ret.id} className="border-b hover:bg-muted/20 transition-colors" data-testid={`return-row-${ret.id}`}>
                      ...
                      <td className="p-3 text-sm">
                        <span className="font-medium">{ret.credit_account_name || ''}</span>
                        <span className="text-muted-foreground ml-1 text-xs">{ret.credit_account_code || ''}</span>
                      </td>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-[98vw] w-full max-h-[95vh] overflow-y-auto" data-testid="sales-return-form-dialog">
          <DialogHeader>
            <DialogTitle>{editingReturn ? 'Edit Sales Return' : 'New Sales Return'}</DialogTitle>
          ...
          <div className="space-y-6 mt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <Label>Sales Return Account (Debit)</Label>
                <AccountSelector
                    ...
                    {formData.lines.map((line, index) => (
                      <tr key={index} className="border-b" data-testid={`line-row-${index}`}>
                        ...
                        <td className="p-2 text-center">
                          <input type="checkbox" checked={line.is_taxable === true} onChange={(e) => handleLineChange(index, 'is_taxable', e.target.checked)} className="w-4 h-4 cursor-pointer" data-testid={`sr-taxable-${index}`} />
                        </td>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  Sales Return: {viewReturn.return_number}
              ...
              <div className="space-y-4 mt-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div><span className="text-muted-foreground">Customer:</span> <strong>{viewReturn.credit_account_name} ({viewReturn.credit_account_code})</strong></div>
                  <div><span className="text-muted-foreground">Return Acc:</span> <strong>{viewReturn.debit_account_name} ({viewReturn.debit_account_code})</strong></div>
