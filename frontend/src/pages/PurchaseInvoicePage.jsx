...
const PurchaseInvoicePage = () => {
  ...
  const fetchInvoices = async (reset = false) => {
    ...
    try {
      ...
      const [invoicesRes, countRes] = await Promise.all([
        ...
        axios.get(`${API}/purchase-invoices/count?organization_id=${currentOrg.id}${filterStatus !== 'all' ? `&status=${filterStatus}` : ''}${searchTerm ? `&search=${searchTerm}` : ''}`)
      ]);
      if (reset) {
  ...
  const handleSave = async () => {
    if (!formData.debit_account_id || !formData.credit_account_id) {
      toast.error('Please select both Purchase and Supplier accounts');
      return;
    }
  ...
  const handlePost = async (id) => {
    ...
    setPosting(true);
    try {
      const res = await axios.post(`${API}/purchase-invoices/${id}/post`);
  ...
  const handleUnpost = async (id) => {
    ...
    setPosting(true);
    try {
      await axios.post(`${API}/purchase-invoices/${id}/unpost`);
  ...
  const handlePrint = (inv) => {
    ...
    pw.document.write(`<!DOCTYPE html><html><head><title>Purchase Invoice ${inv.invoice_number}</title>
      <style>
      ...
      <div class="header">
        ...
        <p>Purchase Invoice / فاتورة مشتريات</p>
      </div>
      <div class="info">
        <div>
          <strong>Invoice #:</strong> ${inv.invoice_number}<br/>
          <strong>Date:</strong> ${formatDate(inv.date)}<br/>
          ${inv.supplier_invoice_number ? `<strong>Supplier Inv #:</strong> ${inv.supplier_invoice_number}<br/>` : ''}
          ${inv.due_date ? `<strong>Due Date:</strong> ${formatDate(inv.due_date)}<br/>` : ''}
        </div>
        <div>
          <strong>Supplier:</strong> ${inv.credit_account_name || ''}<br/>
          <strong>Code:</strong> ${inv.credit_account_code || ''}<br/>
          <strong>Status:</strong> ${inv.status}
        </div>
      </div>
      <table>
        <thead><tr><th>#</th><th>Item</th><th class="text-right">Qty</th><th>Unit</th><th class="text-right">Cost</th><th class="text-right">Sell Price</th><th class="text-right">Disc%</th><th class="text-right">Total</th></tr></thead>
        <tbody>
          ${inv.lines.map((l, i) => `<tr><td>${i+1}</td><td>${l.item_name}${l.item_name_ar ? ' / '+l.item_name_ar : ''}</td><td class="text-right">${l.quantity}</td><td>${l.unit}</td><td class="text-right">${currSymbol} ${parseFloat(l.unit_price).toFixed(3)}</td><td class="text-right">${l.selling_price ? currSymbol+' '+parseFloat(l.selling_price).toFixed(3) : '-'}</td><td class="text-right">${l.discount_percent||0}%</td><td class="text-right">${currSymbol} ${parseFloat(l.line_total).toFixed(3)}</td></tr>`).join('')}
        </tbody>
      </table>
      <table class="totals">
    ...
    <div className="space-y-6" data-testid="purchase-invoice-page">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage purchase invoices and supplier billing</p>
        </div>
        {canEdit() && (
          <Button onClick={openCreateForm} data-testid="create-purchase-invoice-btn">
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full" data-testid="purchase-invoices-table">
                <thead>
                  ...
                  {invoices.map((inv) => (
                    <tr key={inv.id} className="border-b hover:bg-muted/20 transition-colors" data-testid={`invoice-row-${inv.id}`}>
                      ...
                      <td className="p-3 text-sm">
                        <span className="font-medium">{inv.credit_account_name || ''}</span>
                        <span className="text-muted-foreground ml-1 text-xs">{inv.credit_account_code || ''}</span>
                      </td>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-[98vw] w-full max-h-[95vh] overflow-y-auto" data-testid="purchase-invoice-form-dialog">
          <DialogHeader>
            <DialogTitle>{editingInvoice ? 'Edit Purchase Invoice' : 'New Purchase Invoice'}</DialogTitle>
          ...
          <div className="space-y-6 mt-4">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <Input value={formData.supplier_invoice_number} onChange={(e) => setFormData({ ...formData, supplier_invoice_number: e.target.value })} placeholder="Supplier ref..." />
              </div>
              <div>
                <Label>Purchase Account (Debit)</Label>
                <AccountSelector
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  Purchase Invoice: {viewInvoice.invoice_number}
              ...
              <div className="space-y-4 mt-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div><span className="text-muted-foreground">Supplier:</span> <strong>{viewInvoice.credit_account_name} ({viewInvoice.credit_account_code})</strong></div>
                  <div><span className="text-muted-foreground">Purchase Acc:</span> <strong>{viewInvoice.debit_account_name} ({viewInvoice.debit_account_code})</strong></div>
                  ...
                  {viewInvoice.supplier_invoice_number && <div><span className="text-muted-foreground">Supplier Inv#:</span> <strong>{viewInvoice.supplier_invoice_number}</strong></div>}
                  ...
                  <tbody>
                    {viewInvoice.lines.map((line, i) => (
                      <tr key={i} className="border-b">
                        ...
                        <td className="p-2 text-right text-blue-400">{line.selling_price ? formatUSD(line.selling_price) : '-'}</td>
