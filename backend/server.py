...
@api_router.get("/sales-invoices", response_model=List[SalesInvoiceResponse])
async def get_sales_invoices(
    ...
    # Build a customer name lookup for search
    customer_names = {}
    if search:
        # Fetch all customer accounts for name lookup
        customers = await db.accounts.find({'organization_id': organization_id}, {'_id': 0, 'id': 1, 'name': 1, 'code': 1}).to_list(10000)
    ...
    if search:
        ...
        for inv in invoices:
            ...
            if search_lower in (inv.get('notes') or '').lower():
                filtered.append(inv)
                continue
            # Check customer/debit account name
...
@api_router.get("/sales-invoices/count")
async def get_sales_invoices_count(
    ...
    if search:
        ...
        customers = await db.accounts.find({'organization_id': organization_id}, {'_id': 0, 'id': 1, 'name': 1, 'code': 1}).to_list(10000)
        customer_names = {c['id']: f"{c.get('code', '')} {c.get('name', '')}".lower() for c in customers}
...
@api_router.get("/sales-invoices/{invoice_id}", response_model=SalesInvoiceResponse)
async def get_sales_invoice(invoice_id: str, current_user: dict = Depends(get_current_user)):
    ...
    enriched = await enrich_invoice(invoice)
    return SalesInvoiceResponse(**enriched)

@api_router.get("/sales-invoices/last-price/{customer_account_id}/{item_id}")
async def get_last_price_for_customer(
    customer_account_id: str,
    item_id: str,
    current_user: dict = Depends(get_current_user)
):
    """Get the last price at which an item was sold to a specific customer"""
    # Find the most recent posted invoice for this customer that contains this item
    pipeline = [
        {
            '$match': {
...
async def create_sales_invoice(
    ...
    """Create a new sales invoice with multi-currency line items"""
    if current_user['role'] not in ['super_admin', 'admin', 'accountant']:
        raise HTTPException(status_code=403, detail="Permission denied")
    
    invoice_id = str(uuid.uuid4())
    invoice_number = await generate_invoice_number(invoice_data.organization_id)
    
    # Process lines to ensure line_total_usd is calculated
    lines_data = []
    for line in invoice_data.lines:
        line_dict = line.model_dump()
        # Calculate line_total_usd if not provided
    ...
    enriched = await enrich_invoice(invoice_doc)
    return SalesInvoiceResponse(**enriched)

@api_router.put("/sales-invoices/{invoice_id}", response_model=SalesInvoiceResponse)
async def update_sales_invoice(
    invoice_id: str,
    invoice_data: SalesInvoiceUpdate,
    current_user: dict = Depends(get_current_user)
):
    """Update a sales invoice (draft only, or super_admin for posted)"""
    if current_user['role'] not in ['super_admin', 'admin', 'accountant']:
        raise HTTPException(status_code=403, detail="Permission denied")
    ...
    # Only super_admin can edit posted invoices
    if invoice.get('is_posted') and current_user['role'] != 'super_admin':
        raise HTTPException(status_code=403, detail="Only super admin can edit posted invoices")
    
    update_doc = {}
    for field, value in invoice_data.model_dump(exclude_unset=True).items():
        if value is not None:
            if field == 'lines':
                update_doc[field] = [line.model_dump() if hasattr(line, 'model_dump') else line for line in value]
            else:
                update_doc[field] = value
    ...
    enriched = await enrich_invoice(updated)
    return SalesInvoiceResponse(**enriched)

@api_router.delete("/sales-invoices/{invoice_id}")
async def delete_sales_invoice(invoice_id: str, force: bool = False, current_user: dict = Depends(get_current_user)):
    """Delete a sales invoice. Super admin can force delete posted invoices."""
    if current_user['role'] not in ['super_admin', 'admin']:
        raise HTTPException(status_code=403, detail="Permission denied")
    ...
    if invoice.get('is_posted'):
        # Only super_admin can delete posted invoices
        if current_user['role'] != 'super_admin':
            raise HTTPException(status_code=403, detail="Only super admin can delete posted invoices")
        
        if not force:
            raise HTTPException(status_code=400, detail="Cannot delete posted invoice. Use force=true to delete.")
        
        # If force deleting a posted invoice, we need to:
        # 1. Reverse the voucher (restore account balances)
        ...
        if voucher_number:
            voucher = await db.vouchers.find_one({'voucher_number': voucher_number}, {'_id': 0})
            if voucher:
                # Reverse journal entries (restore balances)
                for entry in voucher.get('entries', []):
                    account = await db.accounts.find_one({'code': entry['account_code'], 'organization_id': invoice['organization_id']})
                    if account:
                        # Reverse: if was debit, subtract; if was credit, add
                        update_usd = -entry.get('debit_usd', 0) + entry.get('credit_usd', 0)
                        update_lbp = -entry.get('debit_lbp', 0) + entry.get('credit_lbp', 0)
                        await db.accounts.update_one(
                            {'id': account['id']},
                            {'$inc': {'balance_usd': update_usd, 'balance_lbp': update_lbp}}
                        )
                # Delete the voucher
                await db.vouchers.delete_one({'voucher_number': voucher_number})
        
        # Restore inventory quantities (add back what was sold)
        for line in invoice.get('lines', []):
            if line.get('inventory_item_id'):
                await db.inventory_items.update_one(
                    {'id': line['inventory_item_id']},
                    {'$inc': {'quantity_on_hand': line.get('quantity', 0)}}
                )
    
    await db.sales_invoices.delete_one({'id': invoice_id})
    return {"message": "Invoice deleted successfully"}

@api_router.post("/sales-invoices/{invoice_id}/post")
async def post_sales_invoice(invoice_id: str, current_user: dict = Depends(get_current_user)):
    ...
    voucher_doc = {
        ...
        'currency': invoice.get('currency', 'USD'),  # Default to USD if not specified
    ...
    for line in invoice.get('lines', []):
                ...
                if line.get('batch_id'):
                    for batch in batches:
                        if batch.get('id') == line['batch_id']:
                            ...
                            break
                # Otherwise, use FIFO (deduct from oldest batches first based on expiry_date or received_date)
                elif batches:
                    # Sort batches by expiry_date (FEFO - First Expiry First Out) or received_date
        ...
        for used_item in line.get('used_items', []):
...
@api_router.post("/sales-invoices/{invoice_id}/unpost")
async def unpost_sales_invoice(invoice_id: str, current_user: dict = Depends(get_current_user)):
    ...
    if current_user['role'] != 'super_admin':
        raise HTTPException(status_code=403, detail="Only super admin can unpost invoices")
    ...
    if voucher_id:
        ...
        if voucher:
            # Reverse account balances
            for line in voucher.get('lines', []):
                net_debit = line['debit_usd'] - line['credit_usd']
                net_debit_lbp = line['debit_lbp'] - line['credit_lbp']
                await db.accounts.update_one(
                    {'id': line['account_id']},
                    {'$inc': {'balance_usd': -net_debit, 'balance_lbp': -net_debit_lbp}}
                )
    ...
    for line in invoice.get('lines', []):
        ...
        # Also restore used_items (parts/materials used for services)
        for used_item in line.get('used_items', []):
            if used_item.get('inventory_item_id'):
                await db.inventory_items.update_one(
                    {'id': used_item['inventory_item_id']},
...
async def enrich_purchase_invoice(invoice: dict) -> dict:
    """Add account names to purchase invoice"""
    if invoice.get('debit_account_id'):
        acc = await db.accounts.find_one({'id': invoice['debit_account_id']}, {'code': 1, 'name': 1})
        if acc:
            invoice['debit_account_code'] = acc['code']
            invoice['debit_account_name'] = acc['name']
    
    if invoice.get('credit_account_id'):
        acc = await db.accounts.find_one({'id': invoice['credit_account_id']}, {'code': 1, 'name': 1})
        if acc:
            invoice['credit_account_code'] = acc['code']
            invoice['credit_account_name'] = acc['name']
    
    if invoice.get('voucher_id'):
        voucher = await db.vouchers.find_one({'id': invoice['voucher_id']}, {'voucher_number': 1})
        if voucher:
            invoice['voucher_number'] = voucher['voucher_number']
    
    return invoice

@api_router.get("/purchase-invoices", response_model=List[PurchaseInvoiceResponse])
async def get_purchase_invoices(
    ...
    enriched = []
    for inv in invoices:
        enriched.append(await enrich_purchase_invoice(inv))
    
    return enriched

@api_router.get("/purchase-invoices/count")
...
@api_router.get("/purchase-invoices/{invoice_id}", response_model=PurchaseInvoiceResponse)
async def get_purchase_invoice(invoice_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return await enrich_purchase_invoice(invoice)

@api_router.post("/purchase-invoices", response_model=PurchaseInvoiceResponse)
async def create_purchase_invoice(
    ...
    last_invoice = await db.purchase_invoices.find_one(
        {'organization_id': invoice_data.organization_id, 'invoice_number': {'$regex': f'^{prefix}'}},
        {'invoice_number': 1},
        sort=[('invoice_number', -1)]
    )
    if last_invoice:
        last_num = int(last_invoice['invoice_number'].split('-')[-1])
        invoice_number = f"{prefix}{last_num + 1:05d}"
    else:
        invoice_number = f"{prefix}00001"
    ...
    for line in invoice_data.lines:
        line_dict = line.model_dump()
        if line_dict.get('line_total_usd') is None:
            ex_rate = line_dict.get('exchange_rate', 1) or 1
            if line_dict.get('currency', 'USD') == 'USD':
                line_dict['line_total_usd'] = line_dict['line_total']
            else:
                line_dict['line_total_usd'] = line_dict['line_total'] / ex_rate
    ...
    return await enrich_purchase_invoice(invoice_doc)

@api_router.put("/purchase-invoices/{invoice_id}", response_model=PurchaseInvoiceResponse)
async def update_purchase_invoice(
    ...
    if invoice.get('is_posted'):
        raise HTTPException(status_code=400, detail="Cannot edit posted invoice. Unpost first.")
    ...
    if 'lines' in update_data:
        ...
        for line in update_data['lines']:
            if isinstance(line, dict):
                if line.get('line_total_usd') is None:
                    ex_rate = line.get('exchange_rate', 1) or 1
                    if line.get('currency', 'USD') == 'USD':
                        line['line_total_usd'] = line['line_total']
                    else:
                        line['line_total_usd'] = line['line_total'] / ex_rate
                lines.append(line)
    ...
    return await enrich_purchase_invoice(updated)
...
@api_router.post("/purchase-invoices/{invoice_id}/post")
async def post_purchase_invoice(invoice_id: str, current_user: dict = Depends(get_current_user)):
    ...
    # Generate voucher number (PV = Purchase Voucher)
    year = datetime.now().year
    prefix = f"PV-{year}-"
    last_voucher = await db.vouchers.find_one(
        {'organization_id': invoice['organization_id'], 'voucher_number': {'$regex': f'^{prefix}'}},
    ...
    # Debit: Purchases/Inventory (expense/asset increases)
    # Credit: Supplier Payable (liability increases)
    voucher_id = str(uuid.uuid4())
    voucher_doc = {
        'id': voucher_id,
        'voucher_number': voucher_number,
        'voucher_type': 'PV',  # Purchase Voucher
        'date': invoice['date'],
        ...
        'lines': [
            {
                ...
                'account_code': debit_account['code'],
                'account_name': debit_account['name'],
                'description': f"Purchases from {credit_account['name']}",
                'debit_lbp': amount_lbp,
                'credit_lbp': 0,
                'debit_usd': amount_usd,
                'credit_usd': 0
            },
            {
                ...
                'account_code': credit_account['code'],
                'account_name': credit_account['name'],
                'description': f"Payable to {credit_account['name']}",
                'debit_lbp': 0,
                'credit_lbp': amount_lbp,
                'debit_usd': 0,
                'credit_usd': amount_usd
    ...
    # Update account balances
    # Debit account (Purchases/Inventory) increases
    await db.accounts.update_one(
        {'id': invoice['debit_account_id']},
        {'$inc': {'balance_usd': amount_usd, 'balance_lbp': amount_lbp}}
    )
    # Credit account (Supplier Payable) increases (credit = negative for liabilities display)
    await db.accounts.update_one(
        {'id': invoice['credit_account_id']},
        {'$inc': {'balance_usd': -amount_usd, 'balance_lbp': -amount_lbp}}
    )
...
@api_router.post("/purchase-invoices/{invoice_id}/unpost")
