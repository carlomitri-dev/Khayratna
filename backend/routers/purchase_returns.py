"""
Purchase Returns Router - Handles all purchase return endpoints
Standalone returns with partial quantity support, reversed VAT posting, inventory removal
"""
...
async def get_purchase_returns(
    ...
    if supplier_id: query['debit_account_id'] = supplier_id
    if date_from: query['date'] = {'$gte': date_from}
    if date_to:
        if 'date' in query: query['date']['$lte'] = date_to
        else: query['date'] = {'$lte': date_to}
    if search:
        search_regex = {'$regex': search, '$options': 'i'}
        query['$or'] = [{'return_number': search_regex}, {'reason': search_regex}, {'notes': search_regex}]
    returns = await db.purchase_returns.find(query, {'_id': 0}).sort('date', -1).skip(skip).limit(limit).to_list(limit)
    for ret in returns:
        await enrich_purchase_return(ret)
    return returns
...
async def get_purchase_returns_count(
    ...
    draft_count = await db.purchase_returns.count_documents({**query, 'status': 'draft'})
    posted_count = await db.purchase_returns.count_documents({**query, 'status': 'posted'})
...
async def create_purchase_return(return_data: PurchaseReturnCreate, current_user: dict = Depends(get_current_user)):
    ...
    return_number = f"PR-{year}-{str(count + 1).zfill(5)}"
    ret = {
        'id': str(uuid.uuid4()),
        ...
        **return_data.model_dump(),
        ...
        'updated_at': None, 'posted_at': None, 'posted_by': None,
        'voucher_id': None, 'voucher_number': None
    }
    await db.purchase_returns.insert_one(ret)
    ret.pop('_id', None)
    await enrich_purchase_return(ret)
    return ret
...
async def update_purchase_return(return_id: str, return_data: PurchaseReturnUpdate, current_user: dict = Depends(get_current_user)):
    ...
    await db.purchase_returns.update_one({'id': return_id}, {'$set': update_data})
    updated = await db.purchase_returns.find_one({'id': return_id}, {'_id': 0})
    await enrich_purchase_return(updated)
    return updated
...
async def post_purchase_return(return_id: str, current_user: dict = Depends(get_current_user)):
        ...
        'lines': [
            {
                ...
                'description': f"Reduce payable to {debit_account['name']}",
            ...
            },
            {
                ...
                'description': f"Purchase Return - {credit_account['name']}",
    ...
    for acc_data in [{'code': debit_account['code'], 'net_usd': amount_usd, 'net_lbp': amount_lbp},
        ...
        for i in range(1, len(acc_data['code'])):
            parent_id = code_to_id.get(acc_data['code'][:i])
            if parent_id:
    ...
    return {"message": "Purchase return posted successfully", "voucher_id": voucher_id, "voucher_number": voucher_number}
...
async def unpost_purchase_return(return_id: str, current_user: dict = Depends(get_current_user)):
    ...
    return {"message": "Purchase return unposted successfully"}
